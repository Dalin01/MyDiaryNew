...
public class DialogUpdateAnswer extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        textEntryView = inflater.inflate(R.layout.dialog_update_answer, null);
    ...
    public void update() {
        EditText new_food = (EditText) textEntryView.findViewById(R.id.ffood_new);

        String my_new_food = new_food.getText().toString();

        if (my_new_food.trim().equals("")) {
            ...
            values.put(DiaryContract.DiaryEntry.COLUMN_QUES_ANS, my_new_food);
            String selection = DiaryContract.DiaryEntry._ID + " = ?";
            String[] selectionArgs = {"1"};

            try{
                // Insert the new row, returning the primary key value of the new row
                long newRowId = db.update(DiaryContract.DiaryEntry.TABLE_NAME, values, selection, selectionArgs);
